...
**关键约定**：提交代码前必须执行 `./mvnw compile`（Windows 为 `mvnw.cmd compile`），编译会自动触发 Spotless 插件按照 `style/eclipse-formatter.xml`（阿里 P3C 黄山版规范，Eclipse formatter 格式）格式化代码并添加 License Header。编译通过后不要再次在 IDE 中打开代码文件，避免不同 IDE 配置导致格式差异。
...
- 代码格式由 `style/eclipse-formatter.xml`（Eclipse formatter 格式）定义
...
| `style/eclipse-formatter.xml` | 代码格式化规则（Spotless 使用，阿里 P3C 黄山版） |
