...
> 本指南适用于 OpenContiNew 社区旗下的 ContiNew 系列项目（ContiNew Starter / ContiNew Admin / ContiNew Admin UI 等），社区各项目通用。
...
| 方式 | 说明 |
|:-----|:-----|
| 报告 Bug | 通过 [Issue 表单](https://github.com/continew-org/continew-starter/issues/new/choose) 提交，并附上版本号、复现步骤与错误日志 |
| 建议功能 | 通过 [Feature 表单](https://github.com/continew-org/continew-starter/issues/new/choose) 描述使用场景与期望效果 |
...
ContiNew Starter 遵循阿里巴巴《Java 开发手册(黄山版)》，代码格式以 Spotless 为唯一事实源。`style/` 目录下提供了全套配置文件，建议在 IDE 中导入以辅助开发：
...
1. 确认使用的是[最新版本](https://central.sonatype.com/search?namespace=top.continew.starter)，项目由维护者利用业余时间维护，没有额外精力回溯修复历史版本的问题；
2. 搜索 [Issue 广场](https://continew.top/docs/starter/issue-hub.html) 与 [已有 Issue](https://github.com/continew-org/continew-starter/issues)，避免重复提交；
3. 查阅 [使用指南](https://continew.top/docs/starter/guide/introduction.html) 与 [常见问题](https://continew.top/docs/starter/faq.html)。
...
将 [continew-org/continew-starter](https://github.com/continew-org/continew-starter) Fork 到您的账号下，然后克隆到本地：

```bash
git clone https://github.com/<您的用户名>/continew-starter.git
cd continew-starter
```
...
```bash
git remote add upstream https://github.com/continew-org/continew-starter.git
git fetch upstream
```
...
在所在代码托管平台上向 **dev** 分支创建 Pull Request，并按 [PR 模板](.github/PULL_REQUEST_TEMPLATE.md) 填写说明信息（参考[已合并的 PR](https://github.com/continew-org/continew-starter/pulls?q=is%3Apr+is%3Amerged) 有助于提高合并率）。
...
向 ContiNew Starter 贡献代码即表示您同意您的贡献以 [LGPL-3.0](LICENSE) 许可证进行许可。
